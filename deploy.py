...
with open("MyFirstToken.sol", "r") as f:
    source_code = f.read()
...
compiled_sol = compile_source(
    ...
    import_remappings=[f"@openzeppelin/={os.path.abspath('node_modules/@openzeppelin/')}/"]
)
...
MyFirstToken = w3.eth.contract(abi=abi, bytecode=bytecode)

# Get the latest transaction count
nonce = w3.eth.get_transaction_count(account.address)

# Build transaction
transaction = MyFirstToken.constructor(account.address).build_transaction({
